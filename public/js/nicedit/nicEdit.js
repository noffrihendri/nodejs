...
var nicEditorSelect=bkClass.extend({
	construct:function(D,A,C,B){
		...
		if(!window.opera){
			this.contain.onmousedown=this.control.onmousedown=this.txt.onmousedown=bkLib.cancelEvent}this.margin.noSelect();
	...
	open:function(){
		...
		for(var C=0;C<this.selOptions.length;C++){
			...
			var D=new bkElement("div").setStyle({padding:"0px 4px"}).setContent(B[1]).appendTo(A).noSelect();D.addEvent("click",this.update.closure(this,B[0])).addEvent("mouseover",this.over.closure(this,D)).addEvent("mouseout",this.out.closure(this,D)).setAttributes("id",B[0]);this.pane.append(A);
			if(!window.opera){
...
var nicImageButton=nicEditorAdvancedButton.extend({
	addPane:function(){
		...
		this.addForm({
			...
			height:{type:"text",txt:"Height",style:{width:"150px"}},
